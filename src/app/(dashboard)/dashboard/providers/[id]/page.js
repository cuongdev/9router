...
export default function ProviderDetailPage() {
  ...
  const models = (providerId === "cursor" || providerId === "zed" || providerId === "gemini-web") && liveModels.length > 0
    ? liveModels
    : staticModels;
  ...
  // Live per-connection catalogs (cursor, zed, gemini-web): the static registry
  ...
  // Cursor/gemini-web paths are statement-identical to before; zed adds error surfacing.
  useEffect(() => {
    const isLiveCatalog = providerId === "cursor" || providerId === "zed" || providerId === "gemini-web";
    if (!isLiveCatalog) {
      setLiveModels([]);
      return;
    }
