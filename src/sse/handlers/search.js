...
async function handleSingleProviderSearch(body, providerInput, request, apiKey, settings, apiKeyContext) {
  ...
  while (true) {
    ...
    let credentials;
    try {
      credentials = await getProviderCredentials(providerId, excludeConnectionIds, searchLockKey, { accessPolicy: apiKeyContext?.accessPolicy });
      ...
      if (!credentials && fallbackProviderId) {
        credentials = await getProviderCredentials(fallbackProviderId, excludeConnectionIds, searchLockKey, { accessPolicy: apiKeyContext?.accessPolicy });
        if (credentials) {
